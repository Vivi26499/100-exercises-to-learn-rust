...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(ticket) => ticket,
        Err(TicketNewError::TitleNewError(error)) => panic!("{error}"),
        Err(TicketNewError::DescriptionNewError(_)) => {
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleNewError(
        ...
        if description.is_empty() {
            return Err(TicketNewError::DescriptionNewError(
